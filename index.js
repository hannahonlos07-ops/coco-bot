...
async function sendUnsignedDigest() {
  try {
    ...
    if (outstanding.length === 0) {
      const msg =
        allOutstanding.length === 0
          ? "📋 **End-of-day contract check** — ✅ Everyone has signed. Nothing outstanding today! 🎉"
          : `📋 **End-of-day contract check** — ✅ No contracts unsigned within the last ${DIGEST_MAX_DAYS} days.${oldNote}`;
    ...
    const lines = capped.map(
      (doc, i) => `${i + 1}. ${doc.name || "(untitled contract)"} — unsigned ${timeAgo(doc.date_modified || doc.date_created)}`
    );

    const header = `📋 **End-of-day contract check** — ${outstanding.length} agent(s) still haven't signed:${oldNote}`;
    let footer = "Please follow up. 🙏";
    if (extra > 0) footer = `…and ${extra} more (${DIGEST_MAX_LIST} shown).\n` + footer;
    ...
    // Append the footer to the last chunk if it fits, else its own message.
