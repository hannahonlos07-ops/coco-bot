...
const DIGEST_MAX_LIST = 40; // cap names shown; note the rest
// Only list contracts unsigned within this many days — older ones are treated
// as abandoned and hidden (with a count). 0 = show all, no age filter.
const DIGEST_MAX_DAYS = Number(process.env.PANDADOC_DIGEST_MAX_DAYS ?? "7");
// Role to @mention on the digest only (get the ID: Discord Developer Mode →
// right-click the role → Copy Role ID). Falls back to the global notify role.
...
// PandaDoc names these contracts like "Basic Contract (1x lead order) x Jane Doe"
// — the client's name is the part after the last " x ". Pull just that for a
// clean, name-first list. Falls back to the full title if there's no " x ".
function clientName(docName) {
  if (!docName) return "(unknown)";
  const idx = docName.toLowerCase().lastIndexOf(" x ");
  return (idx !== -1 ? docName.slice(idx + 3) : docName).trim();
}

// Compact age like "2d", "6h", "45m".
...
// opts.mentionRoleId — ping a specific role on THIS message only (used by the
// digest). Falls back to DISCORD_NOTIFY_ROLE_ID (which, if set, pings on every
// message). If neither is present, nothing gets pinged.
...
// Builds and posts the end-of-day list of every unsigned contract. Because a
// Discord message caps at 2000 chars, a long list is split across several
// messages so nobody gets cut off. Uses the document name (which already
// contains the client) — no per-document API calls, so a long list can't trip
// PandaDoc's rate limit.
async function sendUnsignedDigest() {
  try {
    ...
    // Newest first — most recently sent contracts at the top.
    const allOutstanding = [...sent, ...viewed].sort((a, b) => outstandingMs(a) - outstandingMs(b));

    // Age filter: drop contracts unsigned longer than DIGEST_MAX_DAYS (abandoned).
    ...
    const oldNote = hiddenOld > 0 ? ` _(${hiddenOld} older than ${DIGEST_MAX_DAYS} days hidden)_` : "";

    if (outstanding.length === 0) {
      const msg =
        allOutstanding.length === 0
          ? "📋 **8 PM check** — ✅ All signed. Nothing pending."
          : `📋 **8 PM check** — ✅ Nothing unsigned in the last ${DIGEST_MAX_DAYS}d.${oldNote}`;
    ...
    const lines = capped.map(
      (doc, i) => `${i + 1}. **${clientName(doc.name)}** · ${compactAge(doc.date_modified || doc.date_created)}`
    );

    const header = `📋 **Unsigned contracts — 8 PM** · ${outstanding.length} pending`;
    let footer = "";
    if (extra > 0) footer = `_+${extra} more (${DIGEST_MAX_LIST} shown)_`;
    if (hiddenOld > 0) footer += (footer ? " · " : "") + `_+${hiddenOld} older than ${DIGEST_MAX_DAYS}d_`;

    // Pack lines into ≤1900-char messages.
    const messages = [];
    let cur = header;
    ...
    // Append the footer (if any) to the last chunk if it fits, else its own message.
    if (footer) {
    ...
    for (let i = 0; i < messages.length; i++) {
      // Ping the team on the first message only.
      await notifyDiscord(messages[i], i === 0 ? { mentionRoleId: DIGEST_ROLE_ID } : {});
      if (i < messages.length - 1) await sleep(1200); // spacing so Discord doesn't rate-limit
    }
