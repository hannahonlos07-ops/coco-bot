...
// Never nag about contracts older than this — treat them as abandoned/stale.
// Set to 0 to disable the cap. Default 48 hours.
const REMINDER_MAX_AGE_HOURS = Number(process.env.PANDADOC_REMINDER_MAX_AGE_HOURS ?? "48");
...
async function checkOutstandingContracts() {
  ...
  try {
    ...
    const maxAgeMs = REMINDER_MAX_AGE_HOURS > 0 ? REMINDER_MAX_AGE_HOURS * 60 * 60 * 1000 : Infinity;
    let reminders = 0;
    for (const doc of outstanding) {
      if (remindedDocIds.has(doc.id)) continue;
      const age = outstandingMs(doc);
      if (age < thresholdMs) continue; // not overdue yet
      if (age > maxAgeMs) continue; // too old — treat as abandoned, don't nag
...
// On startup, silently record every contract that is ALREADY overdue so we
// never nag about the existing backlog — and, because this runs on every
// boot, so a redeploy/restart doesn't re-flood the same reminders. After
// this, only contracts that newly cross the threshold get a reminder.
async function seedBaseline() {
  ...
  try {
    ...
    console.log(
      `poller: baseline set — ${seeded} already-overdue contract(s) will NOT be re-notified. Only contracts that pass ${REMINDER_HOURS}h from now on will trigger a reminder.`
    );
  } catch (err) {
    console.error("poller: baseline seed failed (will still start polling)", err);
  }
}
...
function startServer() {
  ...
  server.listen(PORT, () => {
    ...
    // Silently baseline the current backlog first (no reminders sent for it),
    // then start the recurring check for newly-overdue contracts.
    (async () => {
      await seedBaseline();
